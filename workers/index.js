...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      } else if (path.startsWith('/api/test-campaign-send')) {
        return await handleTestCampaignSend(request, env);
      } else if (path.startsWith('/api/test-real-campaign')) {
        return await handleTestRealCampaign(request, env);
...
async function handleSendVerification(request, env) {
  ...
  // 🔧 修复：优先使用Resend API发送验证码邮件
  try {
    const resendApiKey = env.RESEND_API_KEY;
    
    console.log('🔍 验证码发送调试信息:');
    console.log('Resend API Key存在:', !!resendApiKey);
    console.log('Resend API Key长度:', resendApiKey ? resendApiKey.length : 0);
    console.log('Resend API Key前缀:', resendApiKey ? resendApiKey.substring(0, 10) + '...' : 'N/A');
    
    // 🔧 修复：强制使用Resend API发送验证码
    console.log('🔧 强制使用Resend API发送验证码');
    ...
    if (resendResponse.ok) {
      ...
      // 如果Resend失败，继续使用Gmail作为后备
      console.log('⚠️ Resend API失败，使用Gmail API作为后备');
    }
...
// 测试Gmail API函数
async function handleTestRealCampaign(request, env) {
  const corsHeaders = {
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
  ...
  if (request.method === 'POST') {
    try {
      console.log('Test Real Campaign: Starting test');
      
      // 模拟真实的campaign发送请求
      const campaignData = {
        subject: 'Test Real Campaign Email',
        businessName: 'Test Company',
        body: '<h1>This is AI-generated content for real campaign test</h1><p>This email was generated by AI and should contain real content.</p>',
        recipients: ['66597405@qq.com']
      };
      ...
      console.log('Test Real Campaign - Campaign data:', JSON.stringify(campaignData, null, 2));
      console.log('Test Real Campaign - User config:', JSON.stringify(userEmailConfig, null, 2));
      
      // 直接调用handleCampaignSend函数
      const result = await handleCampaignSend({
        method: 'POST',
        json: () => Promise.resolve(campaignData),
        headers: {
          get: (name) => {
      ...
      console.log('Test Real Campaign - Result:', result);
      
      return new Response(JSON.stringify({
        success: true,
        message: 'Test real campaign completed',
        result: result,
        timestamp: new Date().toISOString()
      }), {
      ...
      console.error('Test Real Campaign error:', error);
      return new Response(JSON.stringify({
        success: false,
        error: error.message,
        timestamp: new Date().toISOString()
...
async function handleTestCampaignSend(request, env) {
  const corsHeaders = {
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Methods': 'GET, POST, PUT, DELETE, OPTIONS',
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, x-user-email'
  ...
  if (request.method === 'POST') {
    try {
      console.log('Test Campaign Send: Starting test');
      
      // 模拟真实的campaign发送场景
      const campaignData = {
        subject: 'Test Campaign Email',
        businessName: 'Test Company',
        body: '<h1>This is AI-generated content</h1><p>This email was generated by AI and should contain real content.</p>'
      };
      
      const recipients = ['66597405@qq.com'];
      ...
      console.log('Test Campaign Send - Campaign data:', JSON.stringify(campaignData, null, 2));
      console.log('Test Campaign Send - Recipients:', recipients);
      console.log('Test Campaign Send - User config:', JSON.stringify(userEmailConfig, null, 2));
      ...
      for (const recipient of recipients) {
        ...
        console.log('Test Campaign Send - Email data HTML preview:', emailData.html.substring(0, 200) + '...');
        ...
        console.log('Test Campaign Send - SMTP result:', smtpResult);
        
        return new Response(JSON.stringify({
          success: smtpResult.success,
          message: smtpResult.success ? 'Test campaign send successful' : 'Test campaign send failed',
          details: smtpResult,
          timestamp: new Date().toISOString()
        }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
